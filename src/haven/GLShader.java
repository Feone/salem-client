...
public abstract class GLShader implements java.io.Serializable {
	...
	public static class ShaderOb extends GLObject {
		...
		public void compile(GLShader sh) {
			/*
			 * Does JOGL use the byte or char length or the supplied String, and
			 * in case of the former, how does one know the coding it encodes
			 * the String as so as to supply the corrent length? It won't matter
			 * since all reasonable programs will be ASCII, of course, but
			 * still...
			 */
			...
			if (buf[0] != 1) {
				...
				if (buf[0] > 0) {
					...
					/*
					 * The "platform's default charset" is probably a reasonable
					 * choice.
					 */
