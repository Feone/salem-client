...
public class Utils {
	...
	/*
	 * XXX: These are not actually correct, since an exact integer will round
	 * downwards, but I don't actually expect that to be a problem given how I
	 * use these, and it turns out that java.lang.Math.floor is actually
	 * surprisingly slow (it delegates for StrictMath.float for some reason).
	 */
	...
	public static ByteBuffer mkbbuf(int n) {
		try {
			...
			/*
			 * At least Sun's class library doesn't try to collect garbage if
			 * it's out of direct memory, which is pretty stupid. So do it for
			 * it, then.
			 */
			System.gc();
			return (ByteBuffer.allocateDirect(n).order(ByteOrder.nativeOrder()));
	...
	/*
	 * public static ByteBuffer wbbuf(int n) { return(mkbbuf(n)); } public
	 * static IntBuffer wibuf(int n) { return(mkibuf(n)); } public static
	 * FloatBuffer wfbuf(int n) { return(mkfbuf(n)); } public static ShortBuffer
	 * wsbuf(int n) { return(mksbuf(n)); }
	 */
