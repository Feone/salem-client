...
public class CharWnd extends Window {
	...
	public class Attr extends Widget {
		public final Coord imgc = new Coord(0, 1), nmc = new Coord(17, 1), vc = new Coord(137, 1), expc = new Coord(162, 0), expsz = new Coord(sz.x - expc.x - 20, sz.y), btnc = new Coord(sz.x - 17, 0);
		public final String nm;
					...
					if (insp != null) {
						for (int i = 0; i < insp.attrs.length; i++) {
							if (insp.attrs[i].equals(nm)) {
								int w = Math.min(((sz.x - 2) * insp.exp[i]) / cap, sz.x - 2);
								if (insp.exp[i] > cap)
			...
			if (nsk.sel != null) {
				...
				for (int i = 0; i < sk.costa.length; i++) {
					if (sk.costa[i].equals(nm)) {
						int w = Math.min(((sz.x - 2) * sk.costv[i]) / cap, sz.x - 2);
