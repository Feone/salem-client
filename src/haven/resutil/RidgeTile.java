...
public class RidgeTile extends GroundTile {
		...
		public class Tile {
			...
			public void layover(int z, Resource.Tile tile) {
				...
				for (int i = 0; i < n; i++) {
					...
					p.texrot(new Coord((int) (w * planes[i].l), (int) (h * planes[i].u)), new Coord((int) (w * planes[i].r), (int) (h * planes[i].b)), 0, false);
				}
			}
		}
	...
	private static final int[] cwx = { 0, 1, 1, 0 }, cwy = { 0, 0, 1, 1 }, ecwx = { 0, 1, 0, -1 }, ecwy = { -1, 0, 1, 0 };
	...
	private void layend(MapMesh m, Random rnd, Coord lc, Coord gc, int dir) {
		Surface g = m.gnd();
		SPoint bl = g.spoint(lc.add(cwx[dir], cwy[dir])), br = g.spoint(lc.add(cwx[(dir + 1) % 4], cwy[(dir + 1) % 4])), fr = g.spoint(lc.add(cwx[(dir + 2) % 4], cwy[(dir + 2) % 4])), fl = g.spoint(lc.add(cwx[(dir + 3) % 4], cwy[(dir + 3) % 4]));
		...
		m.new Plane(uh, 256, strans[rnd.nextInt(strans.length)].layer(Resource.imgc).tex(), false).texrot(null, null, 1 + dir + (cw ? 2 : 0), false);
		if (cw)
	...
	public void layridge(MapMesh m, Random rnd, Coord lc, Coord gc, boolean[] b) {
		...
		Surface g = m.gnd();
		SPoint ur = g.spoint(lc.add(cwx[dir], cwy[dir])), br = g.spoint(lc.add(cwx[(dir + 1) % 4], cwy[(dir + 1) % 4])), bl = g.spoint(lc.add(cwx[(dir + 2) % 4], cwy[(dir + 2) % 4])), ul = g.spoint(lc.add(cwx[(dir + 3) % 4], cwy[(dir + 3) % 4]));
		SPoint mlu = new SPoint(ul.pos.add(bl.pos).mul(0.5f)), mlb = new SPoint(ul.pos.add(bl.pos).mul(0.5f)), mru = new SPoint(ur.pos.add(br.pos).mul(0.5f)), mrb = new SPoint(ur.pos.add(br.pos).mul(0.5f));
		mlu.pos.z = ul.pos.z;
		mru.pos.z = ur.pos.z;
		mlb.pos.z = bl.pos.z;
		mrb.pos.z = br.pos.z;
		...
		r.set(lc, tile);
		tile.layover(0, set.ground.pick(rnd));
		m.new Plane(upper.vrt, 256, strans[rnd.nextInt(strans.length)].layer(Resource.imgc).tex(), false).texrot(null, null, 3 + dir, false);
	...
	public void laycomplex(MapMesh m, Random rnd, Coord lc, Coord gc, boolean[] b) {
		Surface g = m.gnd();
		SPoint[] crn = { g.spoint(lc), g.spoint(lc.add(1, 0)), g.spoint(lc.add(1, 1)), g.spoint(lc.add(0, 1)), };
		int s;
		for (s = 0; true; s++) {
			if (b[s]) {
				s = (s + 1) % 4;
		...
		for (int i = s, n = 0; n < 4; i = (i + 1) % 4, n++) {
			if (cont) {
				...
				if (pc.pos.z > cc.pos.z) {
					...
					m.new Plane(pl.vrt, 256, strans[rnd.nextInt(strans.length)].layer(Resource.imgc).tex(), false).texrot(null, null, i, false);
				}
			} else {
				...
				if (ub[3] && db[0]) {
					m.new Plane(pl.vrt, 256, c1trans[rnd.nextInt(c1trans.length)].layer(Resource.imgc).tex(), false).texrot(null, null, i, false);
				} else if (!tb[0] && !tb[3] && db[1] && ub[2]) {
					m.new Plane(pl.vrt, 256, c2trans[rnd.nextInt(c2trans.length)].layer(Resource.imgc).tex(), false).texrot(null, null, i, false);
				} else if (ub[3] && !db[0]) {
					Tex t = strans[rnd.nextInt(strans.length)].layer(Resource.imgc).tex();
					m.new Plane(pl.vrt, 256, t, false).texrot(Coord.z, new Coord(t.sz().x / 2, t.sz().y), i, false);
				} else if (!ub[3] && db[0]) {
					Tex t = strans[rnd.nextInt(strans.length)].layer(Resource.imgc).tex();
					m.new Plane(pl.vrt, 256, t, false).texrot(Coord.z, new Coord(t.sz().x / 2, t.sz().y), i + 3, false);
				}
				if (b[(i + 3) % 4]) {
					SPoint pc = ct[(i + 3) % 4], cc = ct[i];
					if (pc.pos.z > cc.pos.z)
