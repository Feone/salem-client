...
public class WaterTile extends Tiler {
				...
				for (int y = ss.ul.y + 1; y < ss.br.y - 1; y++) {
					for (int x = ss.ul.x + 1; x < ss.br.x - 1; x++) {
						if (s[ss.o(x, y)]) {
							...
							sd[ss.o(x, y)] = ((ed[ss.o(x, y)] * 4) + ed[ss.o(x - 1, y)] + ed[ss.o(x + 1, y)] + ed[ss.o(x, y - 1)] + ed[ss.o(x, y + 1)]) / 8;
						}
					}
				}
				ed = sd;
			...
			public void modify(final ProgramContext prog) {
				...
				final ValBlock.Value nmod = prog.fctx.uniform.new Value(Type.VEC3) {
					public Expression root() {
						 ...
						 * vec2(l(0.01), l(0.012))),
						 * mul(Cons.mod(MiscLib.time.ref(), l(2.0)),
						 * vec2(l(0.025), l(0.035))))), "rgb"),
						 * pick(texture2D(snrm.ref(),
						 * add(mul(pick(MiscLib.fragmapv.ref(), "st"),
						 * vec2(l(0.019), l(0.018))),
						 * mul(Cons.mod(add(MiscLib.time.ref(), l(1.0)),
						 * l(2.0)), vec2(l(-0.035), l(-0.025))))), "rgb"),
						 * abs(sub(Cons.mod(MiscLib.time.ref(), l(2.0)),
						 * l(1.0)))), l(0.5)), vec3(l(1.0 / 16), l(1.0 / 16),
						 * l(1.0))));
						 */
						return (mul(
								sub(mix(add(pick(texture2D(snrm.ref(), add(mul(pick(MiscLib.fragmapv.ref(), "st"), vec2(l(0.01), l(0.012))), mul(MiscLib.time.ref(), vec2(l(0.025), l(0.035))))), "rgb"), pick(texture2D(snrm.ref(), add(mul(pick(MiscLib.fragmapv.ref(), "st"), vec2(l(0.019), l(0.018))), mul(MiscLib.time.ref(), vec2(l(-0.035), l(-0.025))))), "rgb")),
										add(pick(texture2D(snrm.ref(), add(mul(pick(MiscLib.fragmapv.ref(), "st"), vec2(l(0.01), l(0.012))), add(mul(MiscLib.time.ref(), vec2(l(0.025), l(0.035))), vec2(l(0.5), l(0.5))))), "rgb"),
												pick(texture2D(snrm.ref(), add(mul(pick(MiscLib.fragmapv.ref(), "st"), vec2(l(0.019), l(0.018))), add(mul(MiscLib.time.ref(), vec2(l(-0.035), l(-0.025))), vec2(l(0.5), l(0.5))))), "rgb")), abs(sub(Cons.mod(MiscLib.time.ref(), l(2.0)), l(1.0)))), l(0.5 * 2)), vec3(l(1.0 / 16), l(1.0 / 16), l(1.0))));
						/*
						 * return(mul(sub(add(pick(texture2D(snrm.ref(),
						 * add(mul(pick(MiscLib.fragmapv.ref(), "st"),
						 * vec2(l(0.01), l(0.012))), mul(MiscLib.time.ref(),
						 * vec2(l(0.025), l(0.035))))), "rgb"),
						 * pick(texture2D(snrm.ref(),
						 * add(mul(pick(MiscLib.fragmapv.ref(), "st"),
						 * vec2(l(0.019), l(0.018))), mul(MiscLib.time.ref(),
						 * vec2(l(-0.035), l(-0.025))))), "rgb")), l(0.5 * 2)),
						 * vec3(l(1.0 / 16), l(1.0 / 16), l(1.0))));
						 */
						/*
						 * return(mul(sub(pick(texture2D(snrm.ref(),
						 * mul(pick(MiscLib.fragmapv.ref(), "st"), l(0.005))),
						 * "rgb"), l(0.5)), vec3(l(1.0 / 32), l(1.0 / 32),
						 * l(1.0))));
						 */
					}
				};
				...
				MiscLib.frageyen(prog.fctx).mod(new Macro1<Expression>() {
					public Expression expand(Expression in) {
						...
						return (add(mul(pick(m, "x"), vec3(l(1.0), l(0.0), l(0.0))), mul(pick(m, "y"), vec3(l(0.0), l(1.0), l(0.0))), mul(pick(m, "z"), in)));
					}
				}, -10);
				prog.fctx.fragcol.mod(new Macro1<Expression>() {
					public Expression expand(Expression in) {
						return (mul(in, textureCube(ssky.ref(), neg(mul(icam.ref(), reflect(MiscLib.fragedir(prog.fctx).depref(), MiscLib.frageyen(prog.fctx).depref())))), l(0.4)));
					}
				}, 0);
			}
		} };
	...
	/*
	 * XXX: This is quite ugly, and tiling should generally be reworked to allow
	 * it to be less so.
	 */
	public static class TWaterTile extends WaterTile {
		...
		public void lay(MapMesh m, Random rnd, Coord lc, Coord gc) {
			...
			for (int i = 0; i < bottom.var.length + 1; i++) {
				...
				if (b.en[i][b.es.o(lc)])
					new BottomPlane(m, m.data(Bottom.id), lc, i, mat, new int[] { (int) (b.bv[i][b.vs.o(lc)] * 255), (int) (b.bv[i][b.vs.o(lc.add(0, 1))] * 255), (int) (b.bv[i][b.vs.o(lc.add(1, 1))] * 255), (int) (b.bv[i][b.vs.o(lc.add(1, 0))] * 255), });
			}
			m.new Plane(m.gnd(), lc, 257, surfmat);
		}
