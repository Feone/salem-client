...
public class BumpMap extends GLState {
		...
		public void modify(ProgramContext prog) {
			final ValBlock.Value nmod = prog.fctx.uniform.new Value(VEC3) {
				public Expression root() {
					return (mul(sub(pick(texture2D(ctex.ref(), Tex2D.texcoord.ref()), "rgb"), l(0.5)), l(2.0)));
				}
			};
			nmod.force();
			MiscLib.frageyen(prog.fctx).mod(new Macro1<Expression>() {
				public Expression expand(Expression in) {
					...
					return (add(mul(pick(m, "s"), tanc.ref()), mul(pick(m, "t"), bitc.ref()), mul(pick(m, "p"), in)));
				}
			}, -100);
			/*
			 * prog.fctx.fragcol.mod(new Macro1<Expression>() { public
			 * Expression expand(Expression in) { return(mix(in,
			 * vec4(nmod.ref(), l(1.0)), l(0.5))); } }, 1000);
			 */
		}
	} };
	...
	public static class MapTangents extends MapMesh.Hooks {
		...
		public void set(MeshBuf buf, Coord lc, MeshBuf.Vertex v1, MeshBuf.Vertex v2, MeshBuf.Vertex v3, MeshBuf.Vertex v4) {
			...
			btan.set(v1, tan[s.o(lc)]);
			bbit.set(v1, bit[s.o(lc)]);
			btan.set(v2, tan[s.o(lc.add(0, 1))]);
			bbit.set(v2, bit[s.o(lc.add(0, 1))]);
			btan.set(v3, tan[s.o(lc.add(1, 1))]);
			bbit.set(v3, bit[s.o(lc.add(1, 1))]);
			btan.set(v4, tan[s.o(lc.add(1, 0))]);
			bbit.set(v4, bit[s.o(lc.add(1, 0))]);
		}
		...
		public void cons(final Resource res, List<GLState> states, List<Material.Res.Resolver> left, Object... args) {
			...
			left.add(new Material.Res.Resolver() {
				public void resolve(Collection<GLState> buf) {
					...
					/*
					 * XXX: It is somewhat doubtful that this cast is really
					 * quite reasonable.
					 */
