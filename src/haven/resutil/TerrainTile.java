...
public class TerrainTile extends Tiler {
			...
			for (int y = es.ul.y; y < es.br.y; y++) {
				for (int x = es.ul.x; x < es.br.x; x++) {
					...
					for (int i = var.length; i >= 0; i--) {
						if (fall) {
							en[i][es.o(x, y)] = false;
						} else if ((bv[i][vs.o(x, y)] < 0.001f) && (bv[i][vs.o(x + 1, y)] < 0.001f) && (bv[i][vs.o(x, y + 1)] < 0.001f) && (bv[i][vs.o(x + 1, y + 1)] < 0.001f)) {
							en[i][es.o(x, y)] = false;
						} else {
							en[i][es.o(x, y)] = true;
							if ((bv[i][vs.o(x, y)] > 0.99f) && (bv[i][vs.o(x + 1, y)] > 0.99f) && (bv[i][vs.o(x, y + 1)] > 0.99f) && (bv[i][vs.o(x + 1, y + 1)] > 0.99f)) {
								fall = true;
							}
						}
					}
	...
	public class Plane extends MapMesh.Shape {
		...
		public Plane(MapMesh m, MapMesh.Surface surf, Coord sc, int z, GLState mat, int[] alpha) {
			...
			vrt = surf.fortile(sc);
			float fac = 25f / 4f;
			tc = new Coord3f[] { new Coord3f((sc.x + 0) / fac, (sc.y + 0) / fac, 0), new Coord3f((sc.x + 0) / fac, (sc.y + 1) / fac, 0), new Coord3f((sc.x + 1) / fac, (sc.y + 1) / fac, 0), new Coord3f((sc.x + 1) / fac, (sc.y + 0) / fac, 0), };
	...
	public void lay(MapMesh m, Random rnd, Coord lc, Coord gc) {
		...
		for (int i = 0; i < var.length + 1; i++) {
			...
			if (b.en[i][b.es.o(lc)])
				new Plane(m, m.gnd(), lc, i, mat, new int[] { (int) (b.bv[i][b.vs.o(lc)] * 255), (int) (b.bv[i][b.vs.o(lc.add(0, 1))] * 255), (int) (b.bv[i][b.vs.o(lc.add(1, 1))] * 255), (int) (b.bv[i][b.vs.o(lc.add(1, 0))] * 255), });
		}
	}
	...
	public class TransPlane extends Plane {
		...
		public TransPlane(MapMesh m, MapMesh.Surface surf, Coord sc, int z, GLState mat, int[] alpha, Tex tex) {
			...
			Coord s = tex.sz();
			cc = new Coord3f[] { new Coord3f(tex.tcx(0), tex.tcy(0), 0), new Coord3f(tex.tcx(0), tex.tcy(s.y), 0), new Coord3f(tex.tcx(s.x), tex.tcy(s.y), 0), new Coord3f(tex.tcx(s.x), tex.tcy(0), 0), };
		}
	...
	private void laytrans(MapMesh m, Coord lc, int z, Tile t) {
		...
		for (int i = 0; i < var.length + 1; i++) {
			...
			if (b.en[i][b.es.o(lc)])
				new TransPlane(m, m.gnd(), lc, z + i, mat, new int[] { (int) (b.bv[i][b.vs.o(lc)] * 255), (int) (b.bv[i][b.vs.o(lc.add(0, 1))] * 255), (int) (b.bv[i][b.vs.o(lc.add(1, 1))] * 255), (int) (b.bv[i][b.vs.o(lc.add(1, 0))] * 255), }, tt);
		}
	}
