...
public class MapView extends PView implements DTarget, Console.Directory {
	...
	static {
		camtypes.put("sucky", FreeCam.class);
	}
	...
	private abstract static class Clicklist<T> extends RenderList {
		...
		protected Color newcol(T t) {
			int cr = ((i & 0x00000f) << 4) | ((i & 0x00f000) >> 12), cg = ((i & 0x0000f0) << 0) | ((i & 0x0f0000) >> 16), cb = ((i & 0x000f00) >> 4) | ((i & 0xf00000) >> 20);
	...
	public void draw(GOut g) {
		...
		try {
			...
			poldraw(g);
			partydraw(g);
			glob.map.reqarea(cc.div(tilesz).sub(MCache.cutsz.mul(view + 1)), cc.div(tilesz).add(MCache.cutsz.mul(view + 1)));
		} catch (Loading e) {
			String text = "Loading...";
