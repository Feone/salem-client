...
public class Outlines implements Rendered {
			...
			Function ofac = new Function.Def(FLOAT) {
				{
					...
					/*
					 * XXX: Current depth detection doesn't work well with
					 * frustum projections, perhaps because of the lack of
					 * precision in the depth buffer (though I'm not sure I buy
					...
					for (int i = 0; i < points.length; i++) {
						...
						if (symmetric) {
							...
							code.add(new If(gt(pick(cross(lnrm, cnrm), "z"), l(0.0)), stmt(aadd(ret, sub(l(1.0), abs(dot(lnrm, cnrm)))))));
						}
					}
			...
			Function msfac = new Function.Def(FLOAT) {
				{
					...
					code.add(new For(ass(i, l(0)), lt(i, FBConfig.numsamples.ref()), linc(i), stmt(aadd(ret, ofac.call(i)))));
	...
	static {
		/*
		 * XXX: It would be good to have some kind of more convenient shader
		 * internation.
		 */
