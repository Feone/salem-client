...
public class MapMesh implements Rendered, Disposable {
	...
	public GLState stfor(Tex tex, boolean clip) {
		...
		if (ret == null) {
			...
			texmap.put(gt, ret = new GLState[] { new Material(Light.deflight, gcol, gt.draw(), gt.clip()), new Material(Light.deflight, gcol, gt.draw()), });
		}
		return (ret[clip ? 0 : 1]);
	}
	...
	public class Surface extends Hooks {
		...
		public SPoint[] fortile(Coord sc) {
			return (new SPoint[] { spoint(sc), spoint(sc.add(0, 1)), spoint(sc.add(1, 1)), spoint(sc.add(1, 0)), });
		}
	}
		...
		public GroundMod(MCache map, DataID<? extends Surface> surf, Tex tex, Coord cc, Coord ul, Coord br) {
			...
			for (t.y = ult.y; t.y <= brt.y; t.y++) {
				for (t.x = ult.x; t.x <= brt.x; t.x++) {
					...
					Coord3f texc = new Coord3f((float) ((t.x * tilesz.x) - ul.x) / (float) (br.x - ul.x), (float) ((t.y * tilesz.y) - ul.y) / (float) (br.y - ul.y), 0);
	...
	private void consflat() {
		...
		flats = new FastMesh[] { new FastMesh(new VertexBuf(posa), ind), new FastMesh(new VertexBuf(posa, cola1), ind), new FastMesh(new VertexBuf(posa, cola2), ind), };
	}
