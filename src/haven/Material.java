...
public class Material extends GLState {
	...
	public static class Colors extends GLState {
		...
		public Colors(Color col) {
			this(new Color((int) (col.getRed() * defamb[0]), (int) (col.getGreen() * defamb[1]), (int) (col.getBlue() * defamb[2]), col.getAlpha()), new Color((int) (col.getRed() * defdif[0]), (int) (col.getGreen() * defdif[1]), (int) (col.getBlue() * defdif[2]), col.getAlpha()), new Color(0, 0, 0, 0), new Color(0, 0, 0, 0), 0);
		}
		...
		public String toString() {
			return (String.format("(%.1f, %.1f, %.1f), (%.1f, %.1f, %.1f), (%.1f, %.1f, %.1f @ %.1f)", amb[0], amb[1], amb[2], dif[0], dif[1], dif[2], spc[0], spc[1], spc[2], shine));
		}
	}
	...
	public static class NewMat implements Resource.LayerFactory<Res> {
		public Res cons(Resource res, byte[] bbuf) {
			...
			while (!buf.eom()) {
				...
				if (nm.equals("linear")) {
					/*
					 * XXX: These should very much be removed and specified
					 * directly in the texture layer instead.
					 */
					ret.linear = true;
