...
public class Light implements Rendered {
	...
	public static class Res extends Resource.Layer {
		...
		private static Color cold(byte[] buf, int[] off) {
			double r, g, b, a;
			r = Utils.floatd(buf, off[0]);
			off[0] += 5;
			g = Utils.floatd(buf, off[0]);
			off[0] += 5;
			b = Utils.floatd(buf, off[0]);
			off[0] += 5;
			a = Utils.floatd(buf, off[0]);
			off[0] += 5;
		...
		public Res(Resource res, byte[] buf) {
			...
			while (off[0] < buf.length) {
				...
				if (t == 1) {
					hatt = true;
					ac = (float) Utils.floatd(buf, off[0]);
					off[0] += 5;
					al = (float) Utils.floatd(buf, off[0]);
					off[0] += 5;
					aq = (float) Utils.floatd(buf, off[0]);
					off[0] += 5;
				} else if (t == 2) {
					...
					hexp = true;
					exp = (float) Utils.floatd(buf, off[0]);
					off[0] += 5;
				} else {
