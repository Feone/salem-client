...
public class Skeleton {
	...
	private static float[] vqrot(float[] d, float[] v, float[] q) {
		...
		/*
		 * I dearly wonder how the JIT's common-subexpression eliminator does on
		 * these.
		 */
		d[0] = (qw * qw * vx) + (2 * qw * qy * vz) - (2 * qw * qz * vy) + (qx * qx * vx) + (2 * qx * qy * vy) + (2 * qx * qz * vz) - (qz * qz * vx) - (qy * qy * vx);
		d[1] = (2 * qx * qy * vx) + (qy * qy * vy) + (2 * qy * qz * vz) + (2 * qw * qz * vx) - (qz * qz * vy) + (qw * qw * vy) - (2 * qw * qx * vz) - (qx * qx * vy);
		d[2] = (2 * qx * qz * vx) + (2 * qy * qz * vy) + (qz * qz * vz) - (2 * qw * qy * vx) - (qy * qy * vz) + (2 * qw * qx * vy) - (qx * qx * vz) + (qw * qw * vz);
		return (d);
	}
	...
	private static float[] qqslerp(float[] d, float[] a, float[] b, float t) {
		...
		if (cos > 0.9999f) {
			/*
			 * Reasonable threshold? Is this function even critical for
			 * performance?
			 */
			d0 = 1.0f - t;
			d1 = t;
	...
	public class Pose {
		...
		public class BoneAlign extends Location {
			...
			public Matrix4f fin(Matrix4f p) {
				if (cseq != seq) {
					...
					/*
					 * System.err.println(cur + ", " + ref + ", " + axis + ", "
					 * + ang);
					 */
					update(Transform.makexlate(new Matrix4f(), new Coord3f(gpos[orig][0], gpos[orig][1], gpos[orig][2])).mul1(Transform.makerot(new Matrix4f(), axis, -ang)));
					cseq = seq;
				}
				return (super.fin(p));
			}
		...
		public void boneoff(int bone, float[] offtrans) {
			/*
			 * It would be nice if these "new float"s get stack-allocated.
			 */
			...
			offtrans[14] = xlate[2];
			/*
			 * I must admit I don't /quite/ understand why the rotation needs to
			 * be inverted...
			 */
	...
	public static class ResPose extends Resource.Layer implements Resource.IDLayer<Integer> {
		...
		private static Track.Frame[] parseframes(byte[] buf, int[] off) {
			Track.Frame[] frames = new Track.Frame[Utils.uint16d(buf, off[0])];
			off[0] += 2;
		...
		private FxTrack parsefx(byte[] buf, int[] off) {
			FxTrack.Event[] events = new FxTrack.Event[Utils.uint16d(buf, off[0])];
			off[0] += 2;
